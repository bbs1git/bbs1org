...
- 插件读写路径使用 `DATA_DIR`、`CACHE_DIR`、`PLUGIN_DIR`、`UPLOAD_DIR` 等核心常量，禁止硬编码部署目录；公开附件地址使用 `upload_url()`。
...
- 建表、补列、索引和卸载分别使用 `app_db_create_table()`、`app_db_ensure_columns()`、`app_db_create_index()`、`app_db_drop_index()`、`app_db_drop_table()`。
...
- 已入库的 NewSMTH、V2EX 主题直接跳过，不再比较或更新字段。只有本次新建采集产生的分页、正文和图片任务可携带明确的 continuation 标记继续处理；历史残留队列仍要重新校验来源键。
