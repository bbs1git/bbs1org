...
这是插件开发的**最上位性约束**。它限制的**不是“渲染钩子里查一次库”**，而是**“查库次数随页内条目数线性放大”**——即逐一逐条造成的 N+1 扩增。判据是量级：查询数若为 **O(页内行数)**（列表逐行、回帖逐条、同一页重复触发）则为违规；若恒为 **O(1)**（整页单次触发、批量预取、内存映射）则合规。

- **违反**：在 `for` / `foreach` / `while` / 列表循环 / 回帖逐条循环内，每渲染一个条目就发一次查询（如 `q()` / `one()` / `val()`），查询数随行数放大。即便用了静态/请求级缓存，只要每个不同 key 的首次查询仍在循环里发生，仍是结构性 N+1，只是被缩小，仍属违规。
- **不违反**：某个钩子整页只触发一次（如 `topic.after_render` 在**主题查看页只对主楼执行一次**、受 `list`/`marker` 限定、只落在详情页单点），且不随行数放大，允许单次查询。这类属于“单点渲染”，可用但建议仍走批量预取。
- **必须避免的误区**：把「详情页单次查询」当成唯一的违规；真正要抓的是那些**在列表或回帖循环里逐条查库**的路径。

平台会对帖子列表、回帖等以“逐条触发渲染钩子”的方式渲染（`topic.after_render` 在列表 / 首页、`reply.after_render` 在每个回帖）。若这些反复触发的路径里每渲染一条就查一次库，整页产生与条目数相等的 N+1，量级从 O(1) 退化为 O(行数)。
...
- `token`：请求内随机串；**同一请求内所有占位符共用一个**。用 `random_bytes()` 生成 6~12 字节再 hex 化，防止把用户输入反馈中伪造的同形注释误当占位符，也避免与真实内容碰撞。
...
2. **合并取回**：在整页钩子里收集对象 ID 去重，用一条 `IN (...)` 批量取回，构建“主键 ID → 渲染结果”映射。
