...
这是插件开发的**最上位硬性约束**，优先级高于其他性能建议：**任何循环、逐条回调或逐条渲染钩子内一律禁止执行数据库操（`q()` / `one()` / `val()` 等）**。

平台对帖子列表、详情页、回帖等多采“逐条触发渲染钩子”的方式（如 `topic.after_render` / `reply.after_render` 由核心对每个对象单独触发一次）。此时若钩子每渲染一条就查一次库，整页会产生与行数相等的 N+1 查询，量级从 O(1) 退化为 O(行数)，在长列表与高访问下尤为致命。

判红标准（触犯即不合格）：
- 在 `for` / `foreach` / `while` / 逐条渲染钩子 / 逐条回调内执行数据库操作。
...
- 已核对所有循环、逐条渲染钩子和回调内不存在 `q()`/`one()`/`val()` 的逐条查询（核心约束）；使用占位符的路径已确认最终 HTML 无残留占位符。
