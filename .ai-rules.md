...
- 界面字号统一使用 CSS 变量：`--font-size-xs` 为 12px、`--font-size-sm` 为 13px、`--font-size-md` 为 14px、`--font-size-lg` 为 16px、`--font-size-xl` 为 18px。`font-size` 与 `font` 中的字号禁止直接写数字；需要例外字号时先定义语义化变量，再使用该变量。
